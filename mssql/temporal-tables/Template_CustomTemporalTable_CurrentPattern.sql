...
(
	...
	rowExpiredAtTimeUtc DATETIME2(2) NULL,
		CONSTRAINT CK_CustomTemporalTable_VL_rowCreatedAtTimeUtc_rowExpiredAtTimeUtc__ExpiredAfterCreated
			CHECK (rowExpiredAtTimeUtc IS NULL OR rowExpiredAtTimeUtc >= rowCreatedAtTimeUtc),
...
WHERE (rowExpiredAtTimeUtc IS NULL);
...
BEGIN;
	...
	(
		...
		FROM deleted
		WHERE
			deleted.rowExpiredAtTimeUtc IS NOT NULL
		...
		FROM inserted
		WHERE
			inserted.rowExpiredAtTimeUtc IS NULL
	)
	BEGIN;
		THROW 50000, N'Updating temporal history is not allowed.', 1;
	END;
...
(
	...
	WHERE
		CustomTemporalTable.rowExpiredAtTimeUtc IS NULL
);
GO
IF (@@ERROR <> 0 OR @@TRANCOUNT <= 0)
BEGIN
...
WHERE
	...
	AND
	(
		CustomTemporalTable.rowExpiredAtTimeUtc > @systemTimeUtc
		OR CustomTemporalTable.rowExpiredAtTimeUtc IS NULL
	);
